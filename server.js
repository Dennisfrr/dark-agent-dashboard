...
// Função auxiliar para criar uma reunião
async function createMeeting(leadId, meetingData) {
    const neo4jSession = await getSession();
    try {
        const result = await neo4jSession.run(`
            MATCH (l:Lead {idWhatsapp: $leadId})-[:HAS_CONVERSATION]->(c:Conversation)
            CREATE (m:Meeting {
                id: randomUUID(),
            ...
            CREATE (c)-[:SCHEDULED_MEETING]->(m)
            SET c.hasMeetingScheduled = true,
                l.nivelDeInteresseReuniao = 'agendado',
                l.dtUltimaAtualizacao = datetime()
            RETURN m { .* } as meeting
        `, {
            leadId,
            scheduledDate: meetingData.scheduledDate,
            meetingType: meetingData.meetingType || 'initial',
            participants: meetingData.participants || [],
            notes: meetingData.notes || '',
            platform: meetingData.platform || 'Meet',
            meetingLink: meetingData.meetingLink || ''
        });

        if (result.records.length > 0) {
            return convertNeo4jProperties(result.records[0].get('meeting'));
        }
        return null;
    } finally {
...
// Endpoint para criar uma nova reunião
app.post('/api/leads/:id/meetings', async (req, res) => {
    ...
    try {
        ...
        if (meeting) {
            ...
            res.status(404).json({ error: "Não foi possível criar a reunião" });
        }
    } catch (error) {
        console.error(`Erro ao criar reunião para lead ${leadId}:`, error);
        res.status(500).json({ error: "Erro interno ao criar reunião", details: error.message });
    }
});

// Endpoint para listar reuniões de um lead
app.get('/api/leads/:id/meetings', async (req, res) => {
    ...
    try {
        const result = await neo4jSession.run(`
            MATCH (l:Lead {idWhatsapp: $leadId})-[:HAS_CONVERSATION]->(c:Conversation)
            MATCH (c)-[:SCHEDULED_MEETING]->(m:Meeting)
            RETURN m { 
                .*,
                isUpcoming: m.scheduledDate > datetime() 
            } as meeting
            ORDER BY m.scheduledDate DESC
        `, { leadId });

        const meetings = result.records.map(record => convertNeo4jProperties(record.get('meeting')));
        res.json(meetings);
    } catch (error) {
        console.error(`Erro ao buscar reuniões do lead ${leadId}:`, error);
        res.status(500).json({ error: "Erro interno ao buscar reuniões", details: error.message });
    } finally {
        await closeSession(neo4jSession);
    }
});

// Endpoint para atualizar status de uma reunião
    ...
    try {
        const result = await neo4jSession.run(`
            ...
            SET m.status = $status,
                ...
                m.notes = CASE WHEN $notes IS NOT NULL THEN $notes ELSE m.notes END
            RETURN m { .* } as meeting
        `, { meetingId, status, notes });

        if (result.records.length > 0) {
            const meeting = convertNeo4jProperties(result.records[0].get('meeting'));
            res.json(meeting);
        } else {
            res.status(404).json({ error: "Reunião não encontrada" });
        }
    } catch (error) {
        console.error(`Erro ao atualizar status da reunião ${meetingId}:`, error);
        res.status(500).json({ error: "Erro interno ao atualizar status da reunião", details: error.message });
    } finally {
        await closeSession(neo4jSession);
    }
});

// Endpoint para obter estatísticas de reuniões
app.get('/api/meetings/stats', async (req, res) => {
    const neo4jSession = await getSession();
    try {
        const result = await neo4jSession.run(`
            MATCH (m:Meeting)
            WITH m.status as status, count(m) as count,
                 sum(CASE WHEN m.scheduledDate > datetime() THEN 1 ELSE 0 END) as upcomingCount
            RETURN collect({
                status: status,
                count: count,
        ...
        const stats = result.records[0].get('stats').map(stat => ({
            status: stat.status,
            count: stat.count.toNumber(),
            upcomingCount: stat.upcomingCount.toNumber()
        }));

        res.json(stats);
    } catch (error) {
        console.error('Erro ao buscar estatísticas de reuniões:', error);
        res.status(500).json({ error: "Erro interno ao buscar estatísticas de reuniões", details: error.message });
    } finally {
        await closeSession(neo4jSession);
    }
});
...
app.get('/api/leads/:id', async (req, res) => {
    ...
    try {
        ...
        const leadResult = await neo4jSession.run(`
            ...
            OPTIONAL MATCH (l)-[:HAS_PLANNER_HISTORY]->(ph:PlannerHistoryEvent)
            // Adicionar match para reuniões
            OPTIONAL MATCH (l)-[:HAS_CONVERSATION]->(c:Conversation)-[:SCHEDULED_MEETING]->(m:Meeting)
            WHERE m.scheduledDate > datetime() // Apenas reuniões futuras
            WITH l, collect(DISTINCT d { .name, .descricao }) AS dores,
                 ...
                 ph,
                 collect(DISTINCT m { .*, isUpcoming: true })[0] as nextMeeting // Pegar a próxima reunião
            WITH l, dores, interesses, solucoesDiscutidas, memoriasConceituais, nextMeeting,
                 ph ORDER BY ph.timestamp DESC LIMIT 5
            WITH l, dores, interesses, solucoesDiscutidas, memoriasConceituais, nextMeeting,
                 collect(ph { .planName, .stepName, .status, .timestamp, .details }) AS plannerHistory
            OPTIONAL MATCH (l)-[:HAS_REFLECTION]->(refl:Reflection)
            WITH l, dores, interesses, solucoesDiscutidas, memoriasConceituais, plannerHistory, nextMeeting,
                 refl ORDER BY refl.timestamp DESC LIMIT 3
            WITH l, dores, interesses, solucoesDiscutidas, memoriasConceituais, plannerHistory, nextMeeting,
                 collect(refl { .summary, .focusType, .timestamp }) AS recentReflections
            OPTIONAL MATCH (l)-[:GENERATED_HYPOTHESIS]->(hyp:Hypothesis)
            WITH l, dores, interesses, solucoesDiscutidas, memoriasConceituais, plannerHistory, recentReflections, nextMeeting,
                 hyp ORDER BY hyp.timestamp DESC LIMIT 3
            WITH l, dores, interesses, solucoesDiscutidas, memoriasConceituais, plannerHistory, recentReflections, nextMeeting,
                 collect(hyp { .interpretation, .confidence, .timestamp }) AS recentHypotheses
            RETURN l {
                .*,
                id: l.idWhatsapp,
                name: l.nome,
                ...
                currentPlanName: l.currentPlanName,
                ...
                plannerHistorySummary: plannerHistory,
                recentReflectionsSummary: recentReflections,
                recentHypothesesSummary: recentHypotheses,
...
app.get('/api/leads/:id/chathistory', async (req, res) => {
    ...
    try {
        const result = await neo4jSession.run(`
            MATCH (l:Lead {idWhatsapp: $leadId})-[:HAS_CONVERSATION]->(c:Conversation)
            MATCH (c)-[:HAS_MESSAGE]->(msg:Message)
            RETURN msg.role AS role, 
                   msg.text AS text, 
                   msg.timestamp AS timestamp, 
                   msg.type as messageType, 
                   msg.toolCallName as toolCallName, 
                   msg.toolCallArgs as toolCallArgs, 
                   msg.toolResponseContent as toolResponseContent
...
// Funções auxiliares para a dashboard
async function getAgentStatus() {
    try {
        const session = getSession();
        const result = await session.run(`
            MATCH (a:Agent)
            RETURN a.status as status, a.lastActive as lastActive, a.activeConversations as activeConversations
        ...
        return {
            ...
            lastActive: convertNeo4jDateToISO(record.get('lastActive')),
            activeConversations: record.get('activeConversations').toNumber()
        };
    } catch (error) {
...
async function getAgentMetrics() {
    try {
        ...
        const result = await session.run(`
            MATCH (a:Agent)
            RETURN 
                a.totalConversations as totalConversations,
                a.successfulConversations as successfulConversations,
                a.averageResponseTime as averageResponseTime,
        ...
        return {
            ...
            averageResponseTime: record.get('averageResponseTime').toNumber(),
            satisfactionScore: record.get('satisfactionScore').toNumber()
        };
    } catch (error) {
        console.error('Erro ao obter métricas do agente:', error);
        throw error;
    }
}
...
async function getActiveConversations() {
    try {
        const session = getSession();
        // Query modificada para incluir mais detalhes da conversa e do lead
        const result = await session.run(`
            MATCH (l:Lead)-[:HAS_CONVERSATION]->(c:Conversation)
            WHERE c.status = 'active'
            RETURN 
                l.idWhatsapp as leadId,
                l.nome as leadName,
                l.email as leadEmail, // NOVO
                c.id as conversationId,
                c.lastMessage as lastMessage,
                c.lastInteraction as lastInteraction,
                c.messageCount as messageCount,
                c.funnelStage as funnelStage, // NOVO
                c.priority as priority,       // NOVO
                c.topic as topic,             // NOVO
                // NOVO: Verifica se uma reunião foi marcada para esta conversa
                exists((c)-[:SCHEDULED_MEETING]->(:Meeting)) as meetingScheduled 
        ...
        return result.records.map(record => ({
            ...
            leadName: record.get('leadName'),
            leadEmail: record.get('leadEmail'), // NOVO
            conversationId: neo4jIdToString(record.get('conversationId')),
            lastMessage: record.get('lastMessage'),
            lastInteraction: convertNeo4jDateToISO(record.get('lastInteraction')),
            messageCount: record.get('messageCount') ? record.get('messageCount').toNumber() : 0,
            funnelStage: record.get('funnelStage') || 'Não definido', // NOVO
            priority: record.get('priority') || 'Normal',             // NOVO
            topic: record.get('topic') || 'Sem tópico',               // NOVO
            meetingScheduled: record.get('meetingScheduled')         // NOVO
        }));
    } catch (error) {
...
async function getAnalytics() {
    try {
        ...
        const result = await session.run(`
            MATCH (a:Agent)
            RETURN 
                a.dailyConversations as dailyConversations,
                a.weeklyConversations as weeklyConversations,
                a.monthlyConversations as monthlyConversations,
        ...
        const record = result.records[0];
        return {
            dailyConversations: record.get('dailyConversations').toNumber(),
            weeklyConversations: record.get('weeklyConversations').toNumber(),
            monthlyConversations: record.get('monthlyConversations').toNumber(),
...
// NOVA FUNÇÃO para buscar mensagens de uma conversa
async function getConversationMessages(conversationId) {
    try {
        const session = getSession();
        const result = await session.run(`
            MATCH (:Conversation {id: $conversationId})-[:HAS_MESSAGE]->(m:Message)
            RETURN m.sender as sender, m.text as text, m.timestamp as timestamp
        ...
        return result.records.map(record => ({
            ...
            timestamp: convertNeo4jDateToISO(record.get('timestamp')),
        }));
    } catch (error) {
        console.error(`Erro ao obter mensagens para a conversa ${conversationId}:`, error);
        throw error;
    }
}

// NOVA FUNÇÃO para métricas de desempenho detalhadas
async function getAgentPerformance() {
    try {
        const session = getSession();
        const result = await session.run(`
            // Contagem de reuniões marcadas
            OPTIONAL MATCH (m:Meeting) WHERE m.status = 'Scheduled'
            WITH count(m) as meetingsScheduled
            
            // Contagem de leads por estágio do funil
            OPTIONAL MATCH (c:Conversation)
            WITH meetingsScheduled, c.funnelStage as stage, count(c) as leadsInStage
            WITH meetingsScheduled, collect({stage: stage, count: leadsInStage}) as funnelDistribution
        ...
        const record = result.records[0];
        return {
            totalConversations: record.get('totalConversations') ? record.get('totalConversations').toNumber() : 0,
            successfulConversations: record.get('successfulConversations') ? record.get('successfulConversations').toNumber() : 0,
            averageResponseTime: record.get('averageResponseTime') ? record.get('averageResponseTime').toNumber() : 0,
            funnelDistribution: record.get('funnelDistribution'),
            meetingsScheduled: record.get('meetingsScheduled').toNumber()
        };

    } catch (error) {
        console.error('Erro ao obter desempenho do agente:', error);
        throw error;
    }
}
...
app.get('/api/agent/status', async (req, res) => {
    try {
        ...
        console.error('Erro ao obter status do agente:', error);
        res.status(500).json({ error: 'Erro ao obter status do agente' });
    }
});
...
app.get('/api/agent/metrics', async (req, res) => {
    try {
        ...
        console.error('Erro ao obter métricas do agente:', error);
        res.status(500).json({ error: 'Erro ao obter métricas do agente' });
    }
});
...
app.get('/api/conversations', async (req, res) => {
    try {
        ...
        console.error('Erro ao obter conversas ativas:', error);
        res.status(500).json({ error: 'Erro ao obter conversas ativas' });
    }
});
    ...
    try {
        ...
        const result = await session.run(`
            ...
            RETURN 
                ...
                a.responseStyle as responseStyle,
                a.language as language,
                a.maxResponseTime as maxResponseTime,
                a.autoReply as autoReply
            ORDER BY a.lastActive DESC
            LIMIT 1
        `);
        await closeSession(session);
        ...
        res.json({
            ...
            responseStyle: record.get('responseStyle'),
            language: record.get('language'),
            maxResponseTime: record.get('maxResponseTime').toNumber(),
            autoReply: record.get('autoReply')
        });
    } catch (error) {
        console.error('Erro ao obter configurações do agente:', error);
        res.status(500).json({ error: 'Erro ao obter configurações do agente' });
    }
});

app.put('/api/agent/settings', async (req, res) => {
    try {
        const { personality, responseStyle, language, maxResponseTime, autoReply } = req.body;
        const session = getSession();
        ...
        await session.run(`
            ...
            SET 
                ...
                a.responseStyle = $responseStyle,
                a.language = $language,
                a.maxResponseTime = $maxResponseTime,
                a.autoReply = $autoReply,
                a.lastUpdated = datetime()
        `, {
            personality,
            responseStyle,
            language,
            maxResponseTime: neo4j.int(maxResponseTime),
            autoReply
        });
        ...
        console.error('Erro ao atualizar configurações do agente:', error);
        res.status(500).json({ error: 'Erro ao atualizar configurações do agente' });
    }
});

// NOVO ENDPOINT para mensagens de uma conversa
...
// NOVO ENDPOINT para desempenho do agente
app.get('/api/agent/performance', async (req, res) => {
    try {
        const performance = await getAgentPerformance();
        res.json(performance);
    } catch (error) {
        res.status(500).json({ error: 'Erro ao obter dados de desempenho do agente' });
    }
});
...
// =========================================================================
// ENDPOINT PARA RESUMO COMPLETO DO LEAD
// =========================================================================
app.get('/api/leads/:id/situacao', async (req, res) => {
    const { id: leadId } = req.params;
    try {
        const situacao = await LeadService.getSituacaoCompleta(leadId);
        res.json(situacao);
    } catch (error) {
        console.error(`Erro ao buscar situação do lead ${leadId}:`, error);
        if (error.message === "Lead não encontrado") {
            res.status(404).json({ error: "Lead não encontrado" });
        } else {
            res.status(500).json({ error: "Erro interno ao buscar situação do lead", details: error.message });
        }
    }
});

// =========================================================================
// NOVOS ENDPOINTS DE ANALYTICS – RECENT ACTIVITIES, SALES TRENDS, LLM USAGE
// =========================================================================

// Endpoint: /api/analytics/recent-activities?limit=6
// Retorna as atividades mais recentes realizadas pelo agente ou sistema.
app.get('/api/analytics/recent-activities', async (req, res) => {
    const limit = parseInt(req.query.limit) || 6;
    const neo4jSession = await getSession();
    try {
        /*
            Estrutura de atividade alvo ‑ ver src/services/types.ts (RecentActivity):
              {
                id: string;
                type: string; // "call" | "email" | "meeting" | "proposal" | "lead" | "sale" | ...
                description: string;
                createdAt: string; // ISO
                user: { name: string; }
              }
            A consulta abaixo tenta coletar as últimas reflexões, reuniões agendadas e leads recém-criadas.
            Ajuste conforme a estrutura real dos seus dados no Neo4j.
        */
        const result = await neo4jSession.run(`
            // Últimas reflexões do agente
            MATCH (r:Reflection)
            OPTIONAL MATCH (u:User {id: r.agentId})
            WITH r.timestamp AS ts,
                 'reflection' AS type,
                 r.summary AS description,
                 coalesce(u.name, 'Agente') AS userName,
                 elementId(r) AS internalId
            UNION ALL
            // Últimas reuniões criadas
            MATCH (m:Meeting)
            WITH m.createdAt AS ts,
                 'meeting' AS type,
                 'Reunião criada' AS description,
                 'Sistema' AS userName,
                 elementId(m) AS internalId
            UNION ALL
            // Últimos leads criados
            MATCH (l:Lead)
            WITH l.dtCriacao AS ts,
                 'lead' AS type,
                 'Novo lead cadastrado' AS description,
                 'Sistema' AS userName,
                 elementId(l) AS internalId
            ORDER BY ts DESC
            LIMIT $limit
            RETURN internalId AS id,
                   type,
                   description,
                   ts AS createdAt,
                   userName
        ...
        const activities = result.records.map(record => ({
            id: neo4jIdToString(record.get('id')),
            type: record.get('type'),
            description: record.get('description'),
            createdAt: convertNeo4jDateToISO(record.get('createdAt'), 'recent-activities'),
            user: { name: record.get('userName') }
        }));

        return res.json(activities);
    } catch (error) {
        console.error('Erro ao buscar atividades recentes:', error.message, error.stack);
        // Retorna array vazio para evitar falha no front-end
        return res.json([]);
    } finally {
        await closeSession(neo4jSession);
    }
...
// Endpoint: /api/analytics/sales-trends
// Retorna o número de vendas e valor agregado por dia (últimos 30 dias)
app.get('/api/analytics/sales-trends', async (req, res) => {
    const neo4jSession = await getSession();
    try {
        const result = await neo4jSession.run(`
            MATCH (s:Sale)
            WHERE s.closedAt IS NOT NULL AND s.value IS NOT NULL
            WITH date(s.closedAt) AS saleDate,
                 sum(toFloat(s.value)) AS totalValue,
                 count(s) AS totalSales
            RETURN saleDate AS date,
                   totalValue,
                   totalSales
            ORDER BY date ASC
        `);
        ...
        const trends = result.records.map(record => ({
            ...
            totalValue: record.get('totalValue').toNumber ? record.get('totalValue').toNumber() : record.get('totalValue'),
            totalSales: record.get('totalSales').toNumber()
        }));

        return res.json(trends);
    } catch (error) {
        console.error('Erro ao buscar sales-trends:', error.message, error.stack);
        return res.json([]);
    } finally {
        await closeSession(neo4jSession);
    }
...
// Retorna consumo de tokens/custo acumulado nos últimos 30 dias (ou total)
app.get('/api/analytics/llm-usage', async (req, res) => {
    try {
        // Se houver logging no Neo4j (ou outro DB) para consumo, implemente aqui.
        // Por ora, retornamos 0 para não quebrar o frontend.
        return res.json({ tokens: 0, cost: 0 });
    } catch (error) {
        console.error('Erro ao buscar llm-usage:', error.message, error.stack);
        return res.json({ tokens: 0, cost: 0 });
    }
});

// =========================================================================
// NOVOS ENDPOINTS DE REUNIÕES – LISTAGEM GERAL
// =========================================================================

// Endpoint: /api/meetings – lista todas as reuniões independentemente do lead
app.get('/api/meetings', async (req, res) => {
    const neo4jSession = await getSession();
    try {
        const result = await neo4jSession.run(`
            MATCH (m:Meeting)
            RETURN m { .*, isUpcoming: m.scheduledDate > datetime() } AS meeting
            ORDER BY m.scheduledDate DESC
        `);
        const meetings = result.records.map(record => convertNeo4jProperties(record.get('meeting')));
        res.json(meetings);
    } catch (error) {
        console.error('Erro ao listar todas as reuniões:', error.message, error.stack);
        res.status(500).json({ error: 'Erro interno ao listar reuniões', details: error.message });
    } finally {
        await closeSession(neo4jSession);
    }
});
